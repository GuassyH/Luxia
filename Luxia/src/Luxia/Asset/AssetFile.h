...
namespace Luxia::Assets {
	...
	public:
		...
		bool Load() { if (!std::filesystem::exists(assetPath)) { LX_CORE_ERROR("srcPath not set"); return false; } return Load(assetPath); }
		bool Save() { if (!std::filesystem::exists(assetPath)) { LX_CORE_ERROR("srcPath not set"); return false; } return Save(assetPath); }
