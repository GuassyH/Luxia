...
namespace Luxia::Platform::OpenGL {
		...
		for (unsigned int i = 0; i < material->GetTextureCount(type); i++) {
			...
			if (!skip) {
				...
				case aiTextureType_DIFFUSE:
					tex->tex_type = Texture::LX_TEXTURE_DIFFUSE;
					break;
				case aiTextureType_SPECULAR:
					tex->tex_type = Texture::LX_TEXTURE_SPECULAR;
					break;
				case aiTextureType_NORMALS:
					tex->tex_type = Texture::LX_TEXTURE_NORMALS;
					break;
				}
	...
	std::shared_ptr<Mesh> GL_Model::processMesh(aiMesh* mesh, const aiScene* ai_scene) {
		...
		newMesh->local_id = static_cast<uint16_t>(meshes.size());
